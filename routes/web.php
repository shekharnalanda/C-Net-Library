...
use App\Http\Controllers\Public\DigitalLibraryController;
use App\Http\Controllers\Public\EnquiryController;
use App\Http\Controllers\Public\HomeController;
use App\Http\Controllers\Public\JobController;
use App\Http\Controllers\Public\SeatAvailabilityController;
...
Route::get('/jobs/{job}/go', [JobController::class, 'go'])->middleware('throttle:60,1')->name('jobs.go');
Route::get('/digital-library', [DigitalLibraryController::class, 'index'])->name('digital-library.index');
Route::get('/digital-library/{resource}/open', [DigitalLibraryController::class, 'open'])->middleware('throttle:60,1')->name('digital-library.open');

Route::middleware('guest')->group(function () {
    Route::get('/login', [LoginController::class, 'create'])->name('login');
    Route::post('/login', [LoginController::class, 'store'])->middleware('throttle:5,1')->name('login.store');
    Route::get('/admin/setup', [FirstAdminSetupController::class, 'create'])->name('admin.setup');
    Route::post('/admin/setup', [FirstAdminSetupController::class, 'store'])->middleware('throttle:3,10')->name('admin.setup.store');
});
Route::post('/logout', [LoginController::class, 'destroy'])->middleware('auth')->name('logout');

Route::middleware(['auth','student'])->prefix('student')->name('student.')->group(function () {
    Route::get('/dashboard', [StudentDashboardController::class, 'index'])->name('dashboard');
    Route::get('/id-card', IdCardController::class)->name('id-card');
...
Route::middleware(['auth','admin','admin.branch'])->prefix('admin')->name('admin.')->group(function () {
    ...
    Route::middleware('permission:attendance.manage')->group(function () {
        ...
        Route::get('/attendance/qr/{token}', [QrAttendanceController::class, 'landing'])->whereUuid('token')->middleware('throttle:60,1')->name('attendance.qr');
        Route::get('/attendance/scan', [QrAttendanceController::class, 'scan'])->name('attendance.scan');
        Route::post('/attendance/scan', [QrAttendanceController::class, 'lookup'])->middleware('throttle:30,1')->name('attendance.scan.lookup');
    ...
    Route::middleware('permission:library.manage')->group(function () {
        ...
        Route::post('/library/categories', [LibraryController::class, 'storeCategory'])->middleware('throttle:20,1')->name('library.categories.store');
        Route::post('/library/books', [LibraryController::class, 'storeBook'])->middleware('throttle:20,1')->name('library.books.store');
        Route::post('/library/books/{book}/copies', [LibraryController::class, 'storeCopy'])->middleware('throttle:30,1')->name('library.copies.store');
        Route::post('/library/issues', [LibraryController::class, 'issue'])->middleware('throttle:30,1')->name('library.issues.store');
    ...
    Route::middleware('permission:jobs.manage')->group(function () { Route::get('/jobs',[AdminJobController::class,'index'])->name('jobs.index'); Route::post('/jobs',[AdminJobController::class,'store'])->middleware('throttle:30,1')->name('jobs.store'); });
    Route::middleware('permission:communications.manage')->group(function () { Route::get('/communications',[CommunicationController::class,'index'])->name('communications.index'); Route::post('/communications/templates',[CommunicationController::class,'store'])->middleware(['global-admin','throttle:20,1'])->name('communications.templates.store'); });
    Route::middleware('permission:staff.manage')->group(function () { Route::get('/staff',[StaffController::class,'index'])->name('staff.index'); Route::post('/staff',[StaffController::class,'store'])->middleware('throttle:30,1')->name('staff.store'); Route::post('/staff/{staff}/attendance',[StaffController::class,'attendance'])->middleware('throttle:30,1')->name('staff.attendance'); Route::patch('/staff/leaves/{staffLeave}',[StaffController::class,'leave'])->middleware('throttle:20,1')->name('staff.leaves.update'); Route::post('/staff/{staff}/payroll',[StaffController::class,'payroll'])->middleware('throttle:20,1')->name('staff.payroll'); });
    Route::middleware(['permission:settings.manage','global-admin'])->group(function () { Route::get('/settings',[SettingsController::class,'index'])->name('settings.index'); Route::patch('/settings',[SettingsController::class,'update'])->middleware('throttle:20,1')->name('settings.update'); Route::get('/cms',[CmsController::class,'index'])->name('cms.index'); Route::patch('/cms/pages/{page}',[CmsController::class,'updatePage'])->middleware('throttle:20,1')->name('cms.pages.update'); Route::post('/cms/faqs',[CmsController::class,'storeFaq'])->middleware('throttle:20,1')->name('cms.faqs.store'); Route::post('/cms/testimonials',[CmsController::class,'storeTestimonial'])->middleware('throttle:20,1')->name('cms.testimonials.store'); Route::post('/cms/gallery',[CmsController::class,'storeGallery'])->middleware('throttle:10,1')->name('cms.gallery.store'); Route::delete('/cms/gallery/{galleryItem}',[CmsController::class,'destroyGallery'])->middleware('throttle:10,1')->name('cms.gallery.destroy'); });
    Route::middleware(['permission:roles.manage','global-admin'])->group(function () { Route::get('/security',[SecurityController::class,'index'])->name('security.index'); Route::patch('/security/roles/{role}',[SecurityController::class,'updateRole'])->middleware('throttle:20,1')->name('security.roles.update'); Route::patch('/security/users/{user}/roles',[SecurityController::class,'updateUserRoles'])->middleware('