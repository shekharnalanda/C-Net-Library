...
<div class="wrap">
    ...
    <div class="grid">
        ...
        <div>
            <div class="card">
                <h2 style="margin-top:0">Student Photo</h2>
                @if($student->photo)
                    <img class="student-photo" src="{{ asset('storage/'.$student->photo) }}" alt="{{ $student->name }} photo">
                @else
                    <div class="photo-placeholder">No Photo<br>Uploaded</div>
                @endif
                <form method="POST" action="{{ route('admin.students.photo.update', $student) }}" enctype="multipart/form-data">
                    @csrf
                    <div class="field"><label>Upload / Replace Photo<input type="file" name="photo" accept=".jpg,.jpeg,.png,.webp,image/jpeg,image/png,image/webp" required></label></div>
                    <div class="muted" style="font-size:12px;margin:-4px 0 12px">JPG, PNG or WebP · Maximum 2 MB · Square/passport photo recommended.</div>
                    <button class="btn alt" type="submit">Save Student Photo</button>
                </form>
            </div>
