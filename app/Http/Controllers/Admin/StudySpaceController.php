...
class StudySpaceController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $allocations = SeatAllocation::query()
            ...
            ->with(['student:id,branch_id,student_code,name,mobile', 'seat.studyHall:id,branch_id,name', 'studySlot:id,branch_id,name,start_time,end_time,duration_hours'])
        ...
        $summary = [
            'halls' => $halls->count(),
            'seats' => $halls->sum('seats_count'),
            'slots' => $slots->where('status', true)->count(),
            'active_allocations' => SeatAllocation::query()
                ->whereHas('student', fn ($q) => $q->whereIn('branch_id', $branchIds))
                ->whereIn('status', ['active', 'reserved'])
                ->whereDate('allocated_from', '<=', today())
                ->where(fn ($q) => $q->whereNull('allocated_to')->orWhereDate('allocated_to', '>=', today()))
                ->count(),
        ];

        return view('admin.study-space.index', compact(
            'branches', 'halls', 'slots', 'plans', 'students', 'allocations', 'summary'
        ));
    }
    ...
    public function storeHall(Request $request): RedirectResponse
    {
        ...
        StudyHall::create($data);

        return back()->with('success', 'Study hall created successfully.');
    }
    ...
    public function updateHall(Request $request, StudyHall $hall): RedirectResponse
    {
        ...
        return back()->with('success', 'Study hall updated.');
    }

    public function storeSeat(Request $request): RedirectResponse
    {
        $data = $request->validate([
            'study_hall_id' => ['required', 'exists:study_halls,id'],
            'seat_no' => ['required', 'string', 'max:40'],
            'seat_type' => ['nullable', 'string', 'max:80'],
            'status' => ['nullable', 'boolean'],
        ]);

        $hall = StudyHall::findOrFail($data['study_hall_id']);
        AdminBranchScope::authorize($request, $hall->branch_id);

        $request->validate([
            'seat_no' => [Rule::unique('seats')->where(fn ($q) => $q->where('study_hall_id', $hall->id))],
        ]);
    ...
    {
        $seat->loadMissing('studyHall');
        AdminBranchScope::authorize($request, $seat->studyHall->branch_id);

        $data = $request->validate([
            'seat_no' => [
                'required', 'string', 'max:40',
                Rule::unique('seats')->where(fn ($q) => $q->where('study_hall_id', $seat->study_hall_id))->ignore($seat->id),
            ],
            'seat_type' => ['nullable', 'string', 'max:80'],
            'status' => ['nullable', 'boolean'],
        ]);
    ...
    public function storeSlot(Request $request): RedirectResponse
    {
        ...
        StudySlot::create($data);

        return back()->with('success', 'Study slot created successfully.');
    }

    public function updateSlot(Request $request, StudySlot $slot): RedirectResponse
    {
        AdminBranchScope::authorize($request, $slot->branch_id);
        ...
        return back()->with('success', 'Study slot updated.');
    }
    ...
    public function storePlan(Request $request): RedirectResponse
    {
        $data = $request->validate([
            ...
            'half_yearly_fee' => ['nullable', 'numeric', 'min:0'],
        ...
        abort_unless((int) $slot->branch_id === (int) $data['branch_id'], 422, 'Study slot does not belong to selected branch.');
        $data['status'] = $request->boolean('status', true);
    ...
    {
        AdminBranchScope::authorize($request, $plan->branch_id);
        ...
        $slot = StudySlot::findOrFail($data['study_slot_id']);
        abort_unless((int) $slot->branch_id === (int) $plan->branch_id, 422, 'Study slot does not belong to plan branch.');
        $data['status'] = $request->boolean('status');
        $plan->update($data);
    ...
    public function allocate(Request $request): RedirectResponse
    {
        ...
        abort_unless((int) $seat->studyHall->branch_id === (int) $student->branch_id, 422, 'Seat is in another branch.');
        abort_unless((int) $slot->branch_id === (int) $student->branch_id, 422, 'Study slot is in another branch.');

        $to = $data['allocated_to'] ?? $data['allocated_from'];
        $conflict = SeatAllocation::query()
            ...
            ->when($slot->start_time && $slot->end_time, function ($q) use ($slot) {
                $q->where(function ($time) use ($slot) {
        ...
        if ($conflict) {
            return back()->withErrors(['seat_id' => 'Selected seat is already allocated for an overlapping period/slot.'])->withInput();
        }

        $membership = $student->memberships()
            ->where('study_slot_id', $slot->id)
    ...
    public function updateAllocation(Request $request, SeatAllocation $allocation): RedirectResponse
    {
        ...
        return back()->with('success', 'Seat allocation updated.');
    }
}
