...
class StudySpaceController extends Controller
{
    ...
    public function storeSlot(Request $r):RedirectResponse{$d=$this->validateSlot($r);AdminBranchScope::authorize($r,(int)$d['branch_id']);$d=$this->normalizeSlot($r,$d);StudySlot::create($d);return back()->with('success','Study slot created successfully.');}
    ...
    {
        $d=$this->validatePlan($r,true);AdminBranchScope::authorize($r,(int)$d['branch_id']);$s=StudySlot::findOrFail($d['study_slot_id']);abort_unless((int)$s->branch_id===(int)$d['branch_id'],422,'Study slot does not belong to selected branch.');$d=$this->normalizePlan($r,$d,true);FeePlan::create($d);return back()->with('success','Fee plan created successfully.');
    }
    public function updatePlan(Request $r,FeePlan $plan):RedirectResponse
    {
        AdminBranchScope::authorize($r,$plan->branch_id);$d=$this->validatePlan($r,false);$s=StudySlot::findOrFail($d['study_slot_id']);abort_unless((int)$s->branch_id===(int)$plan->branch_id,422,'Study slot does not belong to plan branch.');$d=$this->normalizePlan($r,$d,false);$plan->update($d);return back()->with('success','Fee plan updated.');
    }
    private function validatePlan(Request $r,bool $withBranch):array
    {
        $rules=['study_slot_id'=>['required','exists:study_slots,id'],'name'=>['required','string','max:120'],'monthly_fee'=>['required','numeric','min:0'],'quarterly_fee'=>['nullable','numeric','min:0'],'half_yearly_fee'=>['nullable','numeric','min:0'],'yearly_fee'=>['nullable','numeric','min:0'],'admission_fee'=>['nullable','numeric','min:0'],'registration_fee'=>['nullable','numeric','min:0'],'security_deposit'=>['nullable','numeric','min:0'],'late_fee'=>['nullable','numeric','min:0'],'validity_days'=>['required','integer','min:1','max:3660'],'status'=>['nullable','boolean']];if($withBranch)$rules=['branch_id'=>['required','exists:branches,id']]+$rules;return $r->validate($rules);
    }
    private function normalizePlan(Request $r,array $d,bool $creating):array
    {
        foreach(['admission_fee','registration_fee','security_deposit','late_fee'] as $field){$d[$field]=isset($d[$field])&&$d[$field]!==null&&$d[$field]!==''?(float)$d[$field]:0;}
        foreach(['quarterly_fee','half_yearly_fee','yearly_fee'] as $field){$d[$field]=isset($d[$field])&&$d[$field]!==null&&$d[$field]!==''?(float)$d[$field]:null;}
